...
function phoneNumberHTML(name, tagValue) {
  ...
  const phoneNumberLinks = phoneNumbers.map((number) => `<a href="tel:${number}">${number}</a>`).join(', ');
  ...
  if ('sport' in fdata.properties) {
    ...
    for (var sf in sport_facilities) {
      ...
      if (fdata.properties['sport'].indexOf(sf) > -1) {
        ihtml =
          ihtml + '<img src="cicons/' + sport_facilities[sf].icon + '" title="' + sport_facilities[sf]['text'] + '">';
      }
    }
  }
  ...
  if (
    ...
    if ('capacity:caravans' in fdata.properties) {
      ihtml =
        ...
        '<td style="padding: ' +
        padding +
        'px;"><img src="other-icons/caravan.svg" title="' +
        l10n.capacity_caravans +
        '" style="vertical-align:middle"><br><b>' +
    ...
    if ('capacity:tents' in fdata.properties) {
      ihtml =
        ...
        '<td style="padding: ' +
        padding +
        'px;"><img src="other-icons/tent.svg" title="' +
        l10n.capacity_tents +
        '" style="vertical-align:middle"><br><b>' +
    ...
    if (
      'capacity:pitches' in fdata.properties &&
      !('capacity:caravans' in fdata.properties || 'capacity:tents' in fdata.properties)
    ) {
      ihtml =
        ihtml +
        '<td style="padding: ' +
        padding +
        'px;"><img src="other-icons/caravan+tent.svg" title="' +
        l10n.capacity_caravans +
        '" style="vertical-align:middle"><br><b>' +
    ...
    if ('capacity:persons' in fdata.properties) {
      ihtml =
        ...
        '<td style="padding: ' +
        padding +
        'px;"><img src="other-icons/persons.svg" title="' +
        l10n.capacity_persons +
        '" style="vertical-align:middle"><br><b>' +
...
function f2bugInfo(featureData, lang) {
  ...
  if ('site_relation' in featureData.properties) {
    sitelink = genlink(
      'https://www.openstreetmap.org/relation/' + featureData.properties['site_relation'],
      featureData.properties['site_relation'],
    );
...
function editInJOSM(fdata) {
  ...
  if ('bbox' in fdata) {
    ...
    bbox = [
      fdata.geometry['coordinates'][0] - inc,
      fdata.geometry['coordinates'][1] - inc,
      fdata.geometry['coordinates'][0] + inc,
      fdata.geometry['coordinates'][1] + inc,
    ];
  }
  ...
  var url =
    'http://127.0.0.1:8111/load_and_zoom?left=' +
    bbox[0] +
    '&right=' +
    bbox[2] +
    '&top=' +
...
function editInID(fdata) {
  ...
  let queryParameters = ['editor=id', `${osmObjectType}=${osmObjectId}`];
