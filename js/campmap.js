...
var cfeatures = L.tileLayer('camping_features/{z}/{x}/{y}.png', {
  maxZoom: 19,
  minZoom: 18,
});
...
const pointToLayer = function (featureData, latlng) {
  ...
  if (categories.indexOf(featureData.properties['category']) >= 0) {
    icon = attn
      ? public_icons_warn[featureData.properties['category']]
...
function updateSidebars(featureData) {
  ...
  if (isPrivate) {
    ...
    icon = attn
      ? public_icons_warn_selected[featureData.properties.category]
      : public_icons_selected[featureData.properties.category];
  }
...
function gen_facilities4legend() {
  ...
  fhtml +=
    '<img src="' + 'feature-icons/sanitary_dump_station.svg">&nbsp;' + l10n['sanitary_dump_station'] + '<br />\n';
